...
Production-grade instrumentation for Lutris.ai's CanvasKit/Skia renderer, inspired by game engine profiling tools (Unreal Insights, Unity Frame Timing Manager) and browser render engine internals (Chrome Compositor, WebKit/Skia tracing).
...
performance.measure("Scene Render", {
  ...
  detail: {
    devtools: {
      ...
      track: "Lutris.ai Renderer",
      trackGroup: "Lutris.ai",
      color: "primary",
...
This shows up as a dedicated "Lutris.ai Renderer" track in the DevTools Performance panel alongside browser internals. Extremely powerful for production debugging.
...
Instrument the `render()` method with `performance.mark`/`performance.measure` using the Chrome DevTools extensibility API to create a custom "Lutris.ai" track group:
