# `lutris eval` — Figma-like Plugin API for Headless Scripting

## Overview

`bun lutris eval <file> --code '<js>'` executes JavaScript against a `.fig` file with a Figma-compatible `figma` global object. This enables headless scripting, batch operations, AI tool execution, and testing — all without the GUI.
...
bun lutris eval design.fig --code '
...
cat transform.js | bun lutris eval design.fig --stdin

# Write changes back
bun lutris eval design.fig --code '...' --write
bun lutris eval design.fig --code '...' -o modified.fig
```
...
5. **Stdin** — `echo 'return 42' | bun lutris eval test.fig --stdin` → `42`
