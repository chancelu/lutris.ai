...
import { computeSelectionBounds, computeSnap } from '@llc3233149/core'

import type { EditorStore, Tool } from '@/stores/editor'
import type { NodeType, Rect, SceneNode, Vector } from '@llc3233149/core'
...
export function useCanvasInput(
  ...
  function handleSelectDown(e: MouseEvent, sx: number, sy: number, cx: number, cy: number) {
    ...
    // AI Select mode: click adds node to AI context, drag starts marquee for AI context
    if (aiSelectMode.value) {
      ...
      if (hit) {
        addNodeToAIContext(hit.id)
        store.select([hit.id], true) // shift-select style to show visual feedback
      } else {
        // No hit — start marquee drag for AI context area selection
  ...
  function onMouseDown(e: MouseEvent) {
    ...
    if (e.button === 1 || tool === 'HAND' || isSpaceDown.value) {
      startPanDrag(e)
      return
    }
  ...
  function updateHoverCursor(e: MouseEvent) {
    ...
    // Alt+hover temporarily activates measurement mode
    if (e.altKey && store.state.selectedIds.size > 0 && hit && !store.state.measurementMode) {
      store.state._altMeasurement = true
      store.state.measurementMode = true
    } else if (!e.altKey && store.state._altMeasurement) {
      store.state._altMeasurement = false
  ...
  function onMouseUp(e?: MouseEvent) {
    ...
    if (d.type === 'move') handleMoveUp(d)
      ...
      // In AI Select mode, add marquee-enclosed nodes to AI context
      if (aiSelectMode.value) {
        const marquee = store.state.marquee
        if (marquee && marquee.width > 2 && marquee.height > 2) {
          addNodesInRect(marquee)
        }
      }
  ...
  function handleKeyUp(e: KeyboardEvent) {
    ...
    if (e.key === 'Alt' && store.state._altMeasurement) {
      store.state._altMeasurement = false
      store.state.measurementMode = false
    }
  }
