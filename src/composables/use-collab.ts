...
export function useCollab(store: EditorStore) {
  ...
  /** Derive a deterministic room ID from a project ID. */
  function roomIdFromProjectId(projectId: string): string {
    // Simple deterministic hash: take chars from projectId
    let hash = 0
    ...
    let result = ''
    for (let i = 0; i < ROOM_ID_LENGTH; i++) {
      result += ROOM_ID_CHARS[(seed * (i + 1) + i * 7) % ROOM_ID_CHARS.length]
    }
    return `proj-${result}`
  }

  /** Connect to a project's collaboration room. Auto-disconnects from previous. */
  function connectToProject(projectId: string) {
    const roomId = roomIdFromProjectId(projectId)
    if (state.value.roomId === roomId && state.value.connected) return
    if (state.value.connected) disconnect()
    connect(roomId)
  }
