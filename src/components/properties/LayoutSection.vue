...
function getBinding(field: string): string | undefined {
  return node.value.boundVariables[field]
}

function bindField(field: string, variableId: string) {
  store.graph.bindVariable(node.value.id, field, variableId)
  store.requestRender()
}

function unbindField(field: string) {
    ...
    <template v-if="node.layoutMode !== 'NONE'">
      ...
      <template v-if="isFlex">
        <div class="mt-2 flex items-center gap-1.5">
          ...
          <TokenBindButton
            variable-type="FLOAT"
            :bound-variable-id="getBinding('itemSpacing')"
            @bind="bindField('itemSpacing', $event)"
            @unbind="unbindField('itemSpacing')"
          />
          <button
        ...
        <div v-if="!showIndividualPadding && hasUniformPadding()" class="mt-1.5 flex items-center gap-1.5">
          <ScrubInput
          ...
          <TokenBindButton
            variable-type="FLOAT"
            :bound-variable-id="getBinding('paddingTop')"
            @bind="(id: string) => { bindField('paddingTop', id); bindField('paddingRight', id); bindField('paddingBottom', id); bindField('paddingLeft', id) }"
            @unbind="() => { unbindField('paddingTop'); unbindField('paddingRight'); unbindField('paddingBottom'); unbindField('paddingLeft') }"
          />
        </div>
      </template>
