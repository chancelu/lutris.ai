...
function bindField(field: string, variableId: string) {
  const n = node.value
  if (!n) return
  store.graph.bindVariable(n.id, field, variableId)
  store.requestRender()
}
...
function unbindField(field: string) {
  const n = node.value
  if (!n) return
  store.graph.unbindVariable(n.id, field)
  store.requestRender()
}
...
<template>
  <div v-if="active" data-test-id="appearance-section" class="border-b border-border px-3 py-2">
    ...
    <div class="flex gap-1.5">
      ...
      <TokenBindButton
        variable-type="FLOAT"
        :bound-variable-id="getBinding('opacity')"
        @bind="bindField('opacity', $event)"
        @unbind="unbindField('opacity')"
      />
      <template v-if="hasCornerRadius">
        ...
        <TokenBindButton
          v-if="independentCorners !== true"
          variable-type="FLOAT"
          :bound-variable-id="getBinding('cornerRadius')"
          @bind="bindField('cornerRadius', $event)"
          @unbind="unbindField('cornerRadius')"
        />
        <button
