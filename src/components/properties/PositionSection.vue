...
<template>
  <div v-if="active" data-test-id="position-section" class="border-b border-border px-3 py-2">
    ...
    <div v-if="isMulti" class="mt-1.5 flex gap-1.5">
      ...
      <TokenBindButton
        variable-type="FLOAT"
        :bound-variable-id="getBinding('width')"
        @bind="bindField('width', $event)"
        @unbind="unbindField('width')"
      />
      <ScrubInput
        icon="H"
        :model-value="hValue"
      ...
      <TokenBindButton
        variable-type="FLOAT"
        :bound-variable-id="getBinding('height')"
        @bind="bindField('height', $event)"
        @unbind="unbindField('height')"
      />
    </div>
