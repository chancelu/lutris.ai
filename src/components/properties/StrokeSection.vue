...
import type { Color, SceneNode, Stroke } from '@llc3233149/core'
...
function getStrokeColorBinding(index: number): string | undefined {
  const n = activeNode.value
  return n?.boundVariables[`strokes/${index}/color`]
}

function bindStrokeColor(index: number, variableId: string) {
  const n = activeNode.value
  if (!n) return
  store.graph.bindVariable(n.id, `strokes/${index}/color`, variableId)
  store.requestRender()
}

function unbindStrokeColor(index: number) {
  const n = activeNode.value
  if (!n) return
  store.graph.unbindVariable(n.id, `strokes/${index}/color`)
  store.requestRender()
}

function getWeightBinding(): string | undefined {
  return activeNode.value?.boundVariables['strokeWeight']
}

function bindWeight(variableId: string) {
  const n = activeNode.value
  if (!n) return
  store.graph.bindVariable(n.id, 'strokeWeight', variableId)
  store.requestRender()
}

function unbindWeight() {
  const n = activeNode.value
  if (!n) return
  store.graph.unbindVariable(n.id, 'strokeWeight')
  store.requestRender()
}
...
<template>
  <div v-if="active" data-test-id="stroke-section" class="border-b border-border px-3 py-2">
    ...
    >
      ...
      <TokenBindButton
        variable-type="COLOR"
        :bound-variable-id="getStrokeColorBinding(i)"
        @bind="bindStrokeColor(i, $event)"
        @unbind="unbindStrokeColor(i)"
      />
      <button
    ...
    <div v-if="hasStrokes" class="mt-1 flex items-center gap-1.5">
      ...
      <TokenBindButton
        v-if="!activeNode?.independentStrokeWeights"
        variable-type="FLOAT"
        :bound-variable-id="getWeightBinding()"
        @bind="bindWeight($event)"
        @unbind="unbindWeight()"
      />
