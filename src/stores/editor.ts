...
export function createEditorStore() {
  ...
  const state = shallowReactive({
    ...
    sceneVersion: 0,
    loading: false,
    activeRibbonTab: 'panels' as 'panels' | 'code' | 'ai' | 'handoff',
    measurementMode: false,
    _altMeasurement: false,
  ...
  async function restoreFromIDB(): Promise<boolean> {
    try {
      ...
      const file = new File([data], 'Recovered.fig', { type: 'application/octet-stream' })
      await openFigFile(file)
      state.documentName = 'Recovered'
      return true
    } catch {
      return false
    }
