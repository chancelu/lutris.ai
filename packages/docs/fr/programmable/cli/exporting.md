...
```sh
designflow export design.fig                          # PNG (par défaut)
designflow export design.fig -f jpg -s 2 -q 90       # JPG en 2×, qualité 90
designflow export design.fig -f webp -s 3             # WEBP en 3×
designflow export design.fig -f svg                   # SVG vectoriel
```
...
Supporte aussi `--style designflow` pour le format JSX natif (voir [Moteur de rendu JSX](../jsx-renderer)).
...
```sh
designflow export -f png    # capture du canevas actuel
```
