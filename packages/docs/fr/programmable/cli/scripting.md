...
`lutris eval` vous donne accès à l'API complète Figma Plugin dans le terminal. Lisez des nœuds, modifiez des propriétés, créez des formes — puis écrivez les changements dans le fichier.
...
```sh
cat transform.js | lutris eval design.fig --stdin -w
```
...
```sh
lutris eval -c "figma.currentPage.name"
```
