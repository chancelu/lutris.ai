...
**Copier-coller avec Figma** — sélectionnez des nœuds dans Figma, <kbd>⌘</kbd><kbd>C</kbd>, passez à Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Les remplissages, contours, auto-layout, texte, effets, rayons de coins et réseaux vectoriels sont préservés. Fonctionne dans les deux sens.
...
CLI : `Lutris.ai export design.fig -f jsx --style tailwind`
...
```sh
Lutris.ai tree design.fig          # Arbre de nœuds
Lutris.ai find design.fig --type TEXT  # Recherche
Lutris.ai export design.fig -f png     # Rendu
Lutris.ai analyze colors design.fig    # Audit des couleurs
Lutris.ai analyze clusters design.fig  # Motifs répétés
Lutris.ai eval design.fig -c "..."     # API Plugin Figma
```

Lorsque l'application de bureau est lancée, omettez le fichier pour contrôler l'éditeur en direct via RPC :

```sh
Lutris.ai tree                     # Document en direct
Lutris.ai export -f png            # Capture du canevas
```

Toutes les commandes supportent `--json`. Installation : `bun add -g @Lutris.ai/cli`
...
**Web** — disponible sur [app.lutris.ai](https://app.lutris.ai), installable en PWA sur mobile avec interface tactile optimisée.

**Homebrew :**

```sh
brew install Lutris.ai/tap/Lutris.ai
```

## Polices Google Fonts en secours

Lorsqu'une police n'est pas disponible localement, Lutris.ai la récupère automatiquement depuis Google Fonts. Aucune installation manuelle nécessaire lors de l'ouverture de fichiers .fig avec des polices inconnues.
