# Lutris.ai vs Penpot : Comparaison d'architecture et de performances

Pourquoi comparer ? Lutris.ai existe parce que les plateformes de design fermées contrôlent ce qui est possible. Comprendre les différences architecturales montre ce qu'une alternative ouverte et local-first peut faire différemment.
...
Lutris.ai est **~11× plus petit** — et c'est tout l'intérêt. Ce n'est pas une simplification ; c'est une architecture fondamentalement différente.

## 2. Architecture

### Lutris.ai : Client monolithique
...
L'architecture monoprocessus d'Lutris.ai élimine : la latence réseau, l'overhead de sérialisation aux frontières de services, la complexité d'orchestration de conteneurs et l'overhead de requêtes base de données.
...
Lutris.ai réutilise le schéma éprouvé de Figma directement en TypeScript — zéro traduction. Penpot maintient son propre système de types à travers trois langages.
...
Lutris.ai délègue à Yoga (utilisé par React Native sur des milliards d'appareils) en 314 lignes.
...
Lutris.ai a un avantage significatif — il peut lire les fichiers Figma nativement et coller les données du presse-papiers Figma.
...
2. **Export PDF serveur** — service d'export Chromium headless pour PDF (Lutris.ai exporte déjà en SVG nativement)
3. **Système de plugins** — API complète avec exécution sandboxée
4. **Tokens de design** — support natif des design tokens
5. **CSS Grid layout** — implémentation personnalisée (Lutris.ai attend Yoga Grid)
...
Lutris.ai inclut une [commande `eval`](/programmable/cli/scripting) offrant une API Plugin compatible Figma pour le scripting headless. De plus, 90 outils IA sont disponibles via le chat intégré, le serveur MCP (stdio + HTTP) et le CLI. Penpot a un système de plugins avec exécution sandboxée mais pas d'API de scripting headless ni d'intégration MCP.

## Résumé

| Dimension | Gagnant | Pourquoi |
|-----------|---------|----------|
| **Simplicité architecturale** | Lutris.ai | Un processus vs 5+ services |
| **Performance de rendu** | Lutris.ai | CanvasKit direct vs SVG DOM (défaut) ou WASM empaqueté |
| **Maintenabilité du code** | Lutris.ai | ~26K LOC en 1 langage vs 299K en 4+ |
| **Compatibilité Figma** | Lutris.ai | Codec Kiwi natif vs pas de support .fig |
| **Onboarding développeur** | Lutris.ai | TS/Vue vs Clojure/Rust/Docker |
| **Expérience desktop** | Lutris.ai | Tauri natif vs navigateur uniquement |
| **Moteur de layout** | Lutris.ai | Yoga (éprouvé) vs double implémentation |
| **Collaboration** | Égalité | Penpot : serveur avec contrôle d'accès ; Lutris.ai : P2P via Trystero + Yjs |
| **Self-hosting** | Penpot | Prêt Docker vs desktop uniquement |
| **Maturité écosystème** | Penpot | Années de production vs stade précoce |

Lutris.ai est architecturalement plus léger — un renderer CanvasKit monoprocessus en ~26K LOC TypeScript, compatible Figma par conception. Penpot est une plateforme full-stack avec ~299K LOC. Lutris.ai a le scripting headless, **90 outils AI/MCP**, export SVG et une app desktop native.
