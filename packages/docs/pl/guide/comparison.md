# Lutris.ai vs Penpot: Porównanie architektury i wydajności

Dlaczego porównujemy? Lutris.ai istnieje, ponieważ zamknięte platformy projektowe kontrolują co jest możliwe. Zrozumienie różnic architektonicznych pokazuje, co otwarta alternatywa local-first może zrobić inaczej.
...
Lutris.ai jest **~11× mniejszy** — i o to chodzi. To nie uproszczenie; to fundamentalnie inna architektura.

## 2. Architektura

### Lutris.ai: Klient monolityczny
...
Architektura jednoprocesowa Lutris.ai eliminuje: opóźnienie sieci, narzut serializacji, złożoność orkiestracji kontenerów i narzut zapytań do bazy danych.

## 3. Pipeline renderowania

### Lutris.ai: TS → CanvasKit WASM (bezpośrednio)
...
Lutris.ai reutylizuje sprawdzony schemat Figmy (194 definicje Kiwi) bezpośrednio w TypeScript. Penpot utrzymuje własny system typów w trzech językach.
...
Lutris.ai ma znaczącą przewagę — może czytać pliki Figmy natywnie i wklejać dane ze schowka Figmy.
...
2. **Serwerowy eksport PDF** — usługa eksportu headless Chromium dla PDF (Lutris.ai eksportuje już SVG natywnie)
3. **System pluginów** — pełne API z sandboxowanym wykonywaniem
4. **Tokeny projektowe** — natywne wsparcie design tokenów
5. **CSS Grid layout** — własna implementacja (Lutris.ai czeka na Yoga Grid)
6. **Self-hosting** — wdrożenie Docker dla zespołów
7. **Dojrzałość** — lata użytkowania w produkcji

## 11. Scripting i rozszerzalność

Lutris.ai zawiera [komendę `eval`](/programmable/cli/scripting) oferującą API Plugin kompatybilne z Figmą do skryptowania headless. Ponadto 90 narzędzi AI dostępnych przez wbudowany chat, serwer MCP (stdio + HTTP) i CLI. Penpot ma system pluginów z sandboxem, ale bez API skryptowania headless ani integracji MCP.

## Podsumowanie

| Wymiar | Zwycięzca | Dlaczego |
|--------|-----------|----------|
| **Prostota architektoniczna** | Lutris.ai | Jeden proces vs 5+ usług |
| **Wydajność renderowania** | Lutris.ai | Bezpośredni CanvasKit vs SVG DOM (domyślnie) lub WASM pakowany |
| **Utrzymywalność kodu** | Lutris.ai | ~26K LOC w 1 języku vs 299K w 4+ |
| **Kompatybilność z Figmą** | Lutris.ai | Natywny kodek Kiwi vs brak wsparcia .fig |
| **Onboarding developerów** | Lutris.ai | TS/Vue vs Clojure/Rust/Docker |
| **Doświadczenie desktop** | Lutris.ai | Natywne Tauri vs tylko przeglądarka |
| **Silnik layoutu** | Lutris.ai | Yoga (sprawdzony) vs podwójna implementacja |
| **Współpraca** | Remis | Penpot: serwer z kontrolą dostępu; Lutris.ai: P2P przez Trystero + Yjs |
| **Self-hosting** | Penpot | Gotowy Docker vs tylko desktop |
| **Dojrzałość ekosystemu** | Penpot | Lata produkcji vs wczesny etap |

Lutris.ai jest architektonicznie szczuplejszy — jednoprocesowy renderer CanvasKit w ~26K LOC TypeScript, kompatybilny z Figmą z założenia. Penpot to platforma full-stack z ~299K LOC. Lutris.ai ma skryptowanie headless, **90 narzędzi AI/MCP**, eksport SVG i natywną aplikację desktop.
