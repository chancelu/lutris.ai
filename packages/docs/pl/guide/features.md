...
**Kopiuj i wklej z Figmą** — zaznacz węzły w Figmie, <kbd>⌘</kbd><kbd>C</kbd>, przełącz się na Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Wypełnienia, obrysy, auto-layout, tekst, efekty, promienie narożników i sieci wektorowe są zachowane. Działa w obie strony.
...
```sh
Lutris.ai tree design.fig          # Drzewo węzłów
Lutris.ai find design.fig --type TEXT  # Wyszukiwanie
Lutris.ai export design.fig -f png     # Renderowanie
Lutris.ai analyze colors design.fig    # Audyt kolorów
Lutris.ai analyze clusters design.fig  # Powtarzające się wzorce
...
```sh
Lutris.ai tree                     # Aktywny dokument
Lutris.ai export -f png            # Zrzut ekranu canvasu
```

Wszystkie komendy obsługują `--json`. Instalacja: `bun add -g @Lutris.ai/cli`
...
**Web** — działa na [app.lutris.ai](https://app.lutris.ai), instalowalny jako PWA na urządzeniach mobilnych z UI zoptymalizowanym pod dotyk.
...
Gdy czcionka nie jest dostępna lokalnie, Lutris.ai pobiera ją automatycznie z Google Fonts. Nie trzeba ręcznie instalować czcionek przy otwieraniu plików .fig z nieznanymi fontami.
