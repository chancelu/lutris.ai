...
```sh
designflow export design.fig                          # PNG (domyślnie)
designflow export design.fig -f jpg -s 2 -q 90       # JPG w 2×, jakość 90
designflow export design.fig -f webp -s 3             # WEBP w 3×
designflow export design.fig -f svg                   # SVG wektor
```
...
Obsługuje również `--style designflow` dla natywnego formatu JSX (zobacz [Renderer JSX](../jsx-renderer)).
...
```sh
designflow export -f png    # zrzut ekranu bieżącego płótna
```
