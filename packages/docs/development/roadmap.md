...
- App identity: pencil icon, Cargo crate open_pencil, macOS Dock "Lutris.ai"
...
**Delivered:**
- @Lutris.ai/core extracted to packages/core/ (zero DOM deps, Bun workspace)
- @Lutris.ai/cli with headless .fig operations (info, tree, find, export, analyze, node, pages, variables, eval), CanvasKit CPU rasterization, --json output
...
- MCP server (@Lutris.ai/mcp): stdio + HTTP (Hono + Streamable HTTP with sessions), 87 core tools + 3 file management tools (90 total), runs on Bun and Node.js
...
- VitePress documentation site at lutris.ai with i18n (6 languages)
