...
layout: doc
title: AI & Automation
description: Every operation in Lutris.ai is scriptable — AI chat, CLI, JSX renderer, MCP server, real-time collaboration.
---

# AI & Automation

Lutris.ai treats design files as data. Every operation available in the editor — creating shapes, setting fills, managing auto-layout, exporting assets — is also available from the terminal, from AI agents, and from code. No plugins to install, no API keys, no waiting list.
...
Connect Claude Code, Cursor, Windsurf, or any MCP-compatible client to Lutris.ai. The server exposes 90 tools for reading, creating, and modifying designs — the same tools the built-in AI chat uses. Runs over stdio or HTTP with session support.
...
Lutris.ai is the alternative: open source, MIT licensed, every operation scriptable, data stored locally. Your design files are yours — inspect them, transform them, pipe them into CI, feed them to an LLM. No permission needed.
