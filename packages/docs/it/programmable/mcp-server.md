...
Lutris.ai includes an MCP (Model Context Protocol) server that lets AI coding tools — Claude Code, Cursor, Windsurf, etc. — read and modify `.fig` files headlessly.
...
```json [Bun]
{
  "mcpServers": {
    "Lutris.ai": {
      "command": "bun",
      "args": ["/path/to/Lutris.ai/packages/mcp/src/index.ts"]
    }
  }
}
```
```json [Node.js]
{
  "mcpServers": {
    "Lutris.ai": {
      "command": "npx",
      "args": ["tsx", "/path/to/Lutris.ai/packages/mcp/src/index.ts"]
    }
  }
}
```
...
- File operations are limited to `Lutris.ai_MCP_ROOT` (defaults to current working directory)
- CORS is disabled by default; set `Lutris.ai_MCP_CORS_ORIGIN` to allow one origin
- Optional auth token: `Lutris.ai_MCP_AUTH_TOKEN` (client sends `Authorization: Bearer <token>` or `x-mcp-token`)
...
Teach your AI coding agent to use Lutris.ai tools:

```sh
npx skills add Lutris.ai/skills@Lutris.ai
```
