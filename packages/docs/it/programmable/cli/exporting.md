...
```sh
designflow export design.fig                          # PNG (predefinito)
designflow export design.fig -f jpg -s 2 -q 90       # JPG a 2×, qualità 90
designflow export design.fig -f webp -s 3             # WEBP a 3×
designflow export design.fig -f svg                   # SVG vettoriale
```
...
Supporta anche `--style designflow` per il formato JSX nativo (vedi [Renderer JSX](../jsx-renderer)).
...
```sh
designflow export -f png    # screenshot del canvas corrente
```
