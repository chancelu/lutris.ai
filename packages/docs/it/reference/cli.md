---
title: CLI Reference
description: Complete reference for all designflow commands, options, and flags.
---
...
```sh
designflow node [file] --id <id> [--json]
```
...
```sh
designflow variables [file] [options]
```
...
```sh
designflow export [file] [options]
```
...
| `--style` | | JSX style: `designflow` (default), `tailwind` |
...
```sh
designflow analyze colors [file] [options]
```
...
```sh
designflow analyze typography [file] [options]
```
...
```sh
designflow analyze spacing [file] [options]
```
...
```sh
designflow analyze clusters [file] [options]
```
