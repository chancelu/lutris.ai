# Lutris.ai vs Penpot: Confronto architettura e prestazioni

Perché confrontare? Lutris.ai esiste perché le piattaforme di design chiuse controllano ciò che è possibile. Comprendere le differenze architetturali mostra cosa un'alternativa aperta e local-first può fare diversamente.
...
Lutris.ai è **~11× più piccolo** — e questo è il punto. Non è una semplificazione; è un'architettura fondamentalmente diversa.

## 2. Architettura

### Lutris.ai: Client monolitico
...
L'architettura monoprocesso di Lutris.ai elimina: latenza di rete, overhead di serializzazione, complessità di orchestrazione container e overhead di query database.

## 3. Pipeline di rendering

### Lutris.ai: TS → CanvasKit WASM (diretto)
...
Lutris.ai riutilizza lo schema provato di Figma (194 definizioni Kiwi) direttamente in TypeScript. Penpot mantiene un proprio sistema di tipi attraverso tre linguaggi.
...
Lutris.ai ha un vantaggio significativo — può leggere file Figma nativamente e incollare dati dagli appunti di Figma.
...
2. **Export PDF server** — servizio export Chromium headless per PDF (Lutris.ai esporta già SVG nativamente)
3. **Sistema plugin** — API completa con esecuzione sandboxed
4. **Token di design** — supporto nativo design token
5. **CSS Grid layout** — implementazione propria (Lutris.ai attende Yoga Grid)
6. **Self-hosting** — deployment Docker per team
7. **Maturità** — anni di uso in produzione

## 11. Scripting ed estensibilità

Lutris.ai include un [comando `eval`](/programmable/cli/scripting) che fornisce un'API Plugin compatibile Figma per scripting headless. Inoltre, 90 strumenti AI disponibili via chat integrata, server MCP (stdio + HTTP) e CLI. Penpot ha un sistema plugin con esecuzione sandboxed ma senza API di scripting headless né integrazione MCP.

## Riepilogo

| Dimensione | Vincitore | Perché |
|-----------|---------|--------|
| **Semplicità architetturale** | Lutris.ai | Un processo vs 5+ servizi |
| **Prestazioni di rendering** | Lutris.ai | CanvasKit diretto vs SVG DOM (default) o WASM impacchettato |
| **Manutenibilità codice** | Lutris.ai | ~26K LOC in 1 linguaggio vs 299K in 4+ |
| **Compatibilità Figma** | Lutris.ai | Codec Kiwi nativo vs nessun supporto .fig |
| **Onboarding sviluppatori** | Lutris.ai | TS/Vue vs Clojure/Rust/Docker |
| **Esperienza desktop** | Lutris.ai | Tauri nativo vs solo browser |
| **Motore di layout** | Lutris.ai | Yoga (provato) vs doppia implementazione |
| **Collaborazione** | Pareggio | Penpot: server con controllo accessi; Lutris.ai: P2P via Trystero + Yjs |
| **Self-hosting** | Penpot | Docker-ready vs solo desktop |
| **Maturità ecosistema** | Penpot | Anni di produzione vs stadio iniziale |

Lutris.ai è architetturalmente più snello — un renderer CanvasKit monoprocesso in ~26K LOC TypeScript, compatibile Figma per design. Penpot è una piattaforma full-stack con ~299K LOC. Lutris.ai ha scripting headless, **90 strumenti AI/MCP**, export SVG e app desktop nativa.
