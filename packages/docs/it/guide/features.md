...
**Copia e incolla con Figma** — seleziona i nodi in Figma, <kbd>⌘</kbd><kbd>C</kbd>, passa a Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Riempimenti, bordi, auto-layout, testo, effetti, raggi degli angoli e reti vettoriali vengono preservati. Funziona in entrambe le direzioni.
...
```sh
Lutris.ai tree design.fig          # Albero dei nodi
Lutris.ai find design.fig --type TEXT  # Ricerca
Lutris.ai export design.fig -f png     # Render
Lutris.ai analyze colors design.fig    # Audit colori
Lutris.ai analyze clusters design.fig  # Pattern ripetuti
Lutris.ai eval design.fig -c "..."     # Figma Plugin API
```

Quando l'app desktop è in esecuzione, ometti il file per controllare l'editor live tramite RPC:

```sh
Lutris.ai tree                     # Documento live
Lutris.ai export -f png            # Screenshot del canvas
```

Tutti i comandi supportano `--json`. Installazione: `bun add -g @Lutris.ai/cli`
...
**Web** — disponibile su [app.lutris.ai](https://app.lutris.ai), installabile come PWA su mobile con interfaccia ottimizzata per il touch.
...
Quando un font non è disponibile localmente, Lutris.ai lo scarica automaticamente da Google Fonts. Nessuna installazione manuale necessaria quando si aprono file .fig con font non familiari.
