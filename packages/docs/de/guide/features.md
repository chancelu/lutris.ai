...
**Kopieren & Einfügen mit Figma** — Knoten in Figma auswählen, <kbd>⌘</kbd><kbd>C</kbd>, zu Lutris.ai wechseln, <kbd>⌘</kbd><kbd>V</kbd>. Füllungen, Konturen, Auto-Layout, Text, Effekte, Eckenradien und Vektornetzwerke bleiben erhalten. Funktioniert in beide Richtungen.
...
```sh
Lutris.ai tree design.fig          # Knotenbaum
Lutris.ai find design.fig --type TEXT  # Suche
Lutris.ai export design.fig -f png     # Rendern
Lutris.ai analyze colors design.fig    # Farbanalyse
Lutris.ai analyze clusters design.fig  # Wiederholte Muster
...
```sh
Lutris.ai tree                     # Live-Dokument
Lutris.ai export -f png            # Canvas-Screenshot
```

Alle Befehle unterstützen `--json`. Installation: `bun add -g @Lutris.ai/cli`
...
**Web** — läuft unter [app.lutris.ai](https://app.lutris.ai), als PWA auf Mobilgeräten installierbar mit touch-optimierter Oberfläche.
...
Wenn eine Schriftart lokal nicht verfügbar ist, lädt Lutris.ai sie automatisch von Google Fonts. Keine manuelle Installation nötig beim Öffnen von .fig-Dateien mit unbekannten Schriften.
