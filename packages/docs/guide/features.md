...
**Copy & paste with Figma** — select nodes in Figma, <kbd>⌘</kbd><kbd>C</kbd>, switch to Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Fills, strokes, auto-layout, text, effects, corner radii, and vector networks are preserved. Works both ways.
...
```sh
Lutris.ai tree design.fig          # Node tree
Lutris.ai find design.fig --type TEXT  # Search
Lutris.ai export design.fig -f png     # Render
Lutris.ai analyze colors design.fig    # Color audit
Lutris.ai analyze clusters design.fig  # Repeated patterns
...
```sh
Lutris.ai tree                     # Live document
Lutris.ai export -f png            # Screenshot canvas
```

All commands support `--json`. Install: `bun add -g @Lutris.ai/cli`
...
**Web** — runs at [app.lutris.ai](https://app.lutris.ai), installable as a PWA on mobile with touch-optimized UI.
...
When a font isn't available locally, Lutris.ai fetches it from Google Fonts automatically. No manual installation needed when opening .fig files with unfamiliar fonts.
