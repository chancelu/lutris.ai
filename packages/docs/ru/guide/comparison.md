# Lutris.ai vs Penpot: сравнение архитектуры и производительности

Зачем сравнивать? Lutris.ai существует, потому что закрытые дизайн-платформы ограничивают возможности. Понимание архитектурных различий показывает, что может предложить открытая, локальная альтернатива.
...
Lutris.ai **примерно в 11 раз меньше** — и в этом вся суть. Это не упрощение, а принципиально другая архитектура.

## 2. Архитектура

### Lutris.ai: монолитный клиент
...
Однопроцессная архитектура Lutris.ai устраняет:
...
Архитектура Penpot оптимизирована для **многопользовательских серверных развёртываний**. Lutris.ai оптимизирован для **мгновенной локальной производительности**.

## 3. Конвейер рендеринга

### Lutris.ai: TS → CanvasKit WASM (напрямую)
...
- **1 пересечение границы** (JS → WASM), как и у Lutris.ai — но с явными накладными расходами на сериализацию: UUID разбиваются на 4×u32, трансформации на 6×f32, заливки/обводки упаковываются в бинарный формат, базовые свойства группируются в 104-байтную структуру на фигуру
...
Lutris.ai перерисовывает всю область видимости каждый кадр, потому что CanvasKit, вызываемый напрямую из TS, достаточно быстр и не нуждается в тайлинге.

### Вердикт: рендеринг

| Аспект | Lutris.ai | Penpot |
|--------|-------------|--------|
...
Lutris.ai использует проверенную схему Figma (194 определения Kiwi) напрямую в TypeScript — без преобразований. Penpot поддерживает собственную систему типов в Clojure/ClojureScript/Rust, требующую ручной синхронизации между всеми тремя.

## 5. Движок макетов

### Lutris.ai: Yoga WASM (314 строк)
...
Lutris.ai делегирует проверенной библиотеке (Yoga, используется React Native на миллиардах устройств) в 314 строках. Penpot поддерживает ~3 000+ строк собственного кода макета, дублированного на двух языках.
...
Lutris.ai имеет значительное преимущество — он может читать файлы Figma нативно и даже вставлять данные из буфера обмена Figma. Penpot требует ручного экспорта/импорта и не может открывать файлы `.fig`.
...
Подход Lutris.ai проще и с меньшими накладными расходами. Подход Penpot больше подходит для совместной работы (изменения сериализуемы), но ценой сложности.
...
1. **Серверная совместная работа** — централизованное многопользовательское редактирование с WebSockets, учётными записями и контролем доступа (Lutris.ai использует P2P через Trystero + Yjs — без сервера, но и без контроля доступа или сохранения за пределами сессии)
2. **Экспорт PDF** — headless Chromium для серверного PDF-рендеринга (Lutris.ai экспортирует SVG, но пока не PDF)
3. **Система плагинов** — полный API плагинов с изолированным выполнением
4. **Дизайн-токены** — нативная поддержка дизайн-токенов
5. **Макет CSS Grid** — собственная реализация (Lutris.ai ждёт Yoga Grid)
...
Lutris.ai поставляется с [командой `eval`](/programmable/cli/scripting), предоставляющей Figma-совместимый Plugin API для headless-скриптинга — пакетные операции, автоматизированное тестирование и ИИ-модификации работают без GUI. Кроме того, **90 ИИ-инструментов** доступны через встроенный чат, MCP-сервер (stdio + HTTP) и CLI — охватывающие чтение, создание, модификацию, структуру, переменные, векторные пути, анализ (цвета/типографика/отступы/кластеры), сравнение, булевые операции и упорядочивание. Penpot имеет систему плагинов с изолированным выполнением, но без headless-скриптинга или интеграции MCP.

## Итоги

| Аспект | Лидер | Почему |
|-----------|--------|-----|
| **Простота архитектуры** | Lutris.ai | Один процесс vs 5+ сервисов |
| **Производительность рендеринга** | Lutris.ai | Прямой CanvasKit vs SVG DOM (по умолчанию) или бинарно упакованный WASM |
| **Поддерживаемость кода** | Lutris.ai | ~26K строк на 1 языке vs 299K на 4+ языках |
| **Совместимость с Figma** | Lutris.ai | Нативный Kiwi-кодек vs отсутствие поддержки .fig |
| **Онбординг разработчиков** | Lutris.ai | TS/Vue vs Clojure/Rust/Docker |
| **Десктоп** | Lutris.ai | Нативный Tauri vs только браузер |
| **Движок макетов** | Lutris.ai | Yoga (проверенный) vs собственная двойная реализация |
| **Совместная работа** | Ничья | Penpot: серверная с контролем доступа; Lutris.ai: P2P через Trystero + Yjs, без хостинга |
| **Самостоятельный хостинг** | Penpot | Docker-ready vs только десктоп |
| **Зрелость экосистемы** | Penpot | Годы продакшена vs ранняя стадия |

Lutris.ai архитектурно легче — однопроцессный CanvasKit-рендерер в ~26K строках TypeScript, совместимый с Figma по дизайну. Penpot — полноценная платформа с ~299K строками на Clojure, ClojureScript, Rust и SCSS, плюс парк Docker-сервисов. Оба предлагают совместную работу в реальном времени (с разными архитектурами: P2P vs сервер). Penpot имеет экосистему плагинов и серверный экспорт PDF; Lutris.ai имеет Figma-совместимый headless-скриптинг, **90 ИИ/MCP инструментов**, экспорт SVG и нативное десктоп-приложение.
