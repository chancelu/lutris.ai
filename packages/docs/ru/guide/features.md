...
**Копирование и вставка с Figma** — выделите узлы в Figma, <kbd>⌘</kbd><kbd>C</kbd>, переключитесь на Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Заливки, обводки, авто-макет, текст, эффекты, радиусы скругления и векторные сети сохраняются. Работает в обоих направлениях.
...
```sh
Lutris.ai tree design.fig          # Дерево узлов
Lutris.ai find design.fig --type TEXT  # Поиск
Lutris.ai export design.fig -f png     # Рендеринг
Lutris.ai analyze colors design.fig    # Аудит цветов
Lutris.ai analyze clusters design.fig  # Повторяющиеся паттерны
...
```sh
Lutris.ai tree                     # Текущий документ
Lutris.ai export -f png            # Скриншот канваса
```

Все команды поддерживают `--json`. Установка: `bun add -g @Lutris.ai/cli`
...
**Веб** — работает на [app.lutris.ai](https://app.lutris.ai), устанавливается как PWA на мобильных устройствах с адаптированным интерфейсом.
...
Когда шрифт недоступен локально, Lutris.ai автоматически загружает его из Google Fonts. Ручная установка не требуется при открытии файлов .fig с незнакомыми шрифтами.
