...
Lutris.ai использует JSX как язык создания дизайна. LLM видели миллионы React-компонентов — описание макета через `<Frame><Text>` естественно и не требует специального обучения. Каждый токен важен, когда ИИ-агент выполняет десятки операций, а JSX — это самое компактное декларативное представление.
...
```sh
Lutris.ai export design.fig -f jsx                   # формат Lutris.ai
Lutris.ai export design.fig -f jsx --style tailwind  # классы Tailwind
```
