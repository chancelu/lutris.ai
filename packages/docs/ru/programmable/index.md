...
description: Каждая операция в Lutris.ai доступна из скриптов — ИИ-чат, CLI, JSX-рендерер, MCP-сервер, совместная работа в реальном времени.
---

# ИИ и автоматизация

Lutris.ai работает с дизайн-файлами как с данными. Каждая операция, доступная в редакторе — создание фигур, настройка заливок, управление автолейаутом, экспорт ассетов — также доступна из терминала, ИИ-агентов и кода. Без плагинов, без API-ключей, без листов ожидания.
...
Подключите Claude Code, Cursor, Windsurf или любой MCP-совместимый клиент к Lutris.ai. Сервер предоставляет 90 инструментов для чтения, создания и изменения дизайна — те же инструменты, которые использует встроенный ИИ-чат. Работает через stdio или HTTP с поддержкой сессий.
...
Lutris.ai — альтернатива: открытый исходный код, лицензия MIT, каждая операция доступна из скриптов, данные хранятся локально. Ваши дизайн-файлы — ваши: просматривайте, трансформируйте, отправляйте в CI, загружайте в LLM. Без чьего-либо разрешения.
