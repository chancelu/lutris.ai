...
```sh
designflow export design.fig                          # PNG (по умолчанию)
designflow export design.fig -f jpg -s 2 -q 90       # JPG в 2×, качество 90
designflow export design.fig -f webp -s 3             # WEBP в 3×
designflow export design.fig -f svg                   # SVG-вектор
```
...
Также поддерживается `--style designflow` для нативного JSX-формата (см. [JSX-рендерер](../jsx-renderer)).
...
```sh
designflow export -f png    # снимок текущего холста
```
