# Lutris.ai vs Penpot: Comparación de arquitectura y rendimiento

¿Por qué comparar? Lutris.ai existe porque las plataformas de diseño cerradas controlan lo que es posible. Entender las diferencias arquitectónicas muestra lo que una alternativa abierta y local-first puede hacer diferente.
...
Lutris.ai es **~11× más pequeño** — y ese es el punto. No es una simplificación; es una arquitectura fundamentalmente diferente.

## 2. Arquitectura

### Lutris.ai: cliente monolítico
...
La arquitectura de proceso único de Lutris.ai elimina: latencia de red entre frontend y backend, overhead de serialización/deserialización en los límites de servicios, complejidad de orquestación de contenedores y overhead de consultas de base de datos.

## 3. Pipeline de renderizado

### Lutris.ai: TS → CanvasKit WASM (directo)
...
Lutris.ai reutiliza el esquema probado de Figma (194 definiciones Kiwi) directamente en TypeScript — cero traducción. Penpot mantiene su propio sistema de tipos a través de Clojure/ClojureScript/Rust, requiriendo sincronización manual entre los tres.
...
Lutris.ai delega en una librería probada (Yoga, usada por React Native en miles de millones de dispositivos) en 314 líneas. Penpot mantiene ~3.000+ LOC de código de layout personalizado duplicado.
...
Lutris.ai tiene una ventaja significativa — puede leer archivos Figma nativamente y pegar datos del portapapeles de Figma.
...
El enfoque de Lutris.ai es más simple y con menos overhead.
...
2. **Exportación PDF en servidor** — servicio de export Chromium headless para PDF (Lutris.ai ya exporta SVG nativamente)
3. **Sistema de plugins** — API completa con ejecución sandboxed
4. **Tokens de diseño** — soporte nativo de design tokens
5. **CSS Grid layout** — implementación propia (Lutris.ai espera Yoga Grid)
...
Lutris.ai incluye un [comando `eval`](/programmable/cli/scripting) que proporciona una API de Plugin compatible con Figma para scripting headless. Además, 90 herramientas AI disponibles vía chat integrado, servidor MCP (stdio + HTTP) y CLI. Penpot tiene sistema de plugins con ejecución sandboxed pero sin API de scripting headless ni integración MCP.

## Resumen

| Dimensión | Ganador | Por qué |
|-----------|---------|---------|
| **Simplicidad arquitectónica** | Lutris.ai | Un proceso vs 5+ servicios |
| **Rendimiento de renderizado** | Lutris.ai | CanvasKit directo vs SVG DOM (default) o WASM empaquetado |
| **Mantenibilidad del código** | Lutris.ai | ~26K LOC en 1 lenguaje vs 299K en 4+ |
| **Compatibilidad Figma** | Lutris.ai | Codec Kiwi nativo vs sin soporte .fig |
| **Onboarding de desarrolladores** | Lutris.ai | TS/Vue vs Clojure/Rust/Docker |
| **Experiencia desktop** | Lutris.ai | Tauri nativo vs solo navegador |
| **Motor de layout** | Lutris.ai | Yoga (probado) vs implementación dual personalizada |
| **Colaboración** | Empate | Penpot: servidor con control de acceso; Lutris.ai: P2P vía Trystero + Yjs, zero hosting |
| **Self-hosting** | Penpot | Listo para Docker vs solo desktop |
| **Madurez del ecosistema** | Penpot | Años de producción vs etapa temprana |

Lutris.ai es arquitectónicamente más ligero — un renderer CanvasKit de proceso único en ~26K LOC de TypeScript, compatible con Figma por diseño. Penpot es una plataforma full-stack con ~299K LOC. Ambos ofrecen colaboración en tiempo real (P2P vs servidor). Penpot tiene ecosistema de plugins y exportación PDF en servidor; Lutris.ai tiene scripting headless compatible con Figma, **90 herramientas AI/MCP**, exportación SVG y app desktop nativa.
