...
**Copiar y pegar con Figma** — selecciona nodos en Figma, <kbd>⌘</kbd><kbd>C</kbd>, cambia a Lutris.ai, <kbd>⌘</kbd><kbd>V</kbd>. Rellenos, trazos, auto-layout, texto, efectos, radios de esquina y redes vectoriales se preservan. Funciona en ambas direcciones.
...
```sh
Lutris.ai tree design.fig          # Árbol de nodos
Lutris.ai find design.fig --type TEXT  # Buscar
Lutris.ai export design.fig -f png     # Renderizar
Lutris.ai analyze colors design.fig    # Auditoría de colores
Lutris.ai analyze clusters design.fig  # Patrones repetidos
...
```sh
Lutris.ai tree                     # Documento en vivo
Lutris.ai export -f png            # Captura del canvas
```

Todos los comandos soportan `--json`. Instalar: `bun add -g @Lutris.ai/cli`
...
**Web** — funciona en [app.lutris.ai](https://app.lutris.ai), instalable como PWA en móvil con interfaz optimizada para táctil.
...
Cuando una fuente no está disponible localmente, Lutris.ai la carga automáticamente desde Google Fonts. No requiere instalación manual al abrir archivos .fig con fuentes desconocidas.
