...
```sh
designflow export design.fig                          # PNG (predeterminado)
designflow export design.fig -f jpg -s 2 -q 90       # JPG a 2×, calidad 90
designflow export design.fig -f webp -s 3             # WEBP a 3×
designflow export design.fig -f svg                   # SVG vectorial
```
...
También soporta `--style designflow` para el formato JSX nativo (ver [Renderizador JSX](../jsx-renderer)).
...
```sh
designflow export -f png    # captura de pantalla del lienzo actual
```
