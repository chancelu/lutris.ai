---
title: Exporting
description: Exporting images (PNG, JPG, WEBP) and saving/opening .fig files in Lutris.ai.
---
...
- **Copy as JSX** — copies the Lutris.ai JSX representation (compatible with `renderJsx()`)

## .fig File Operations

Lutris.ai uses the .fig format for full documents — the same binary format as Figma.
...
Files exported from Lutris.ai can be opened in Figma, and vice versa. The .fig format preserves all node types, properties, fills, strokes, effects, vector data, and layout settings.
