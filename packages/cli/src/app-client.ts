import { AUTOMATION_HTTP_PORT } from '@llc3233149/core'
...
export async function getAppToken(): Promise<string> {
  ...
  if (!res || !res.ok) {
    throw new Error(
      `Could not connect to Lutris app on localhost:${AUTOMATION_HTTP_PORT}.\n` +
  ...
  if (data.status !== 'ok' || !data.token) {
    throw new Error(
      'Lutris app is running but no document is open.\n' +
