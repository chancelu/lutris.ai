#!/usr/bin/env bun
/**
 * Visual comparison pipeline: Figma vs Lutris renderer.
 *
 ...
 *   ours.png   — rendered by Lutris headless SkiaRenderer
...
async function runWithClipboard() {
  ...
  console.log('🖼️  Rendering with Lutris…')
...
async function renderFigmaViaPaste() {
  ...
  const _tmpPageId = JSON.parse(createPage.text().trim())
