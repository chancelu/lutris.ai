...
│  │  │              File Format Layer                        │ │  │
│  │  │                                                      │ │  │
│  │  │  .Lutris.ai (Kiwi binary, same codec as .fig)       │ │  │
...
.fig file
  ...
  → apply to Lutris.ai scene graph
```

#### Rendering compatibility

Both Lutris.ai and Figma use Skia CanvasKit for rendering. This means identical rendering primitives. The pixel-perfect challenge is in:
...
Crawl Figma files → for each:
  ...
  2. Import .fig into Lutris.ai
  
  3. Render same node in Lutris.ai:
     Lutris.ai render <id> --scale 2 --output actual.png
...
│   │   ├── node-1-2.png
│   │   └── ...
├── actual/                       # Screenshots from Lutris.ai (test run)
├── diffs/                        # Visual diffs
...
# 2. Render: import .fig into Lutris.ai, render same nodes
...
We already have the full Kiwi codec from figma-use. The `.Lutris.ai` format will use the same encoding:
...
Migration from .fig: decode with our Kiwi codec → re-encode as .Lutris.ai.
...
Tauri v2 desktop app (macOS, Windows, Linux). Web app at app.lutris.ai. CLI published on npm. Documentation site at lutris.ai. GitHub Actions CI for builds and deploys.
