...
- [x] 1.1 `guide/comparison.md` — update scripting section: "75 tools" → "78 tools", mention analyze_*/diff_* tools; update Summary paragraph tool counts; remove "Server-side export" from "What Penpot Does Better" (Lutris.ai now has SVG export); update rendering engine LOC (~3,200 after renderer split)
...
- [x] 3.3 `user-guide/layers-and-pages.md` — add double-click inline rename in Layers Panel section (blur/Enter/Escape to commit); add mobile bottom drawer note; update Code tab description: mention format toggle (Lutris.ai JSX / Tailwind CSS v4)
...
- [x] 4.3 `guide/getting-started.md` — add Homebrew install section before "Building from Source": `brew install open-pencil/tap/lutris`; add `bun run test:unit` row to scripts table
