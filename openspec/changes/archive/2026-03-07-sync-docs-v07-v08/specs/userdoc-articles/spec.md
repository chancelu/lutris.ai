...
The layers-and-pages article's Code Tab description SHALL mention the format toggle: Lutris.ai JSX (custom components) vs Tailwind CSS v4 (HTML with utility classes).

### Scenario: Format toggle mentioned
- **WHEN** user reads the Properties Panel → Code Tab description
- **THEN** both output formats are mentioned (Lutris.ai and Tailwind)
