...
- **THEN** `brew install open-pencil/tap/lutris` is shown as a macOS install option
...
The `guide/comparison.md` (vs Penpot) SHALL reflect the current tool count (78), correct renderer LOC after the renderer split (~3,200 LOC across 10 files), and accurate scripting section. The "What Penpot Does Better" section SHALL NOT list server-side SVG export as a Penpot advantage since Lutris.ai now exports SVG.

### Scenario: comparison.md tool counts current
- **WHEN** user reads the comparison with Penpot
- **THEN** Lutris.ai tool count is stated as 78 (not 75)
