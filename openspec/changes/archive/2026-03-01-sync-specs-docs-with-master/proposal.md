...
- `eval-command`: CLI command `lutris eval <file> --code '<js>'` for executing JavaScript against .fig files. Supports stdin input, writing changes back, and returning JSON results. Enables batch operations, AI tool execution, and headless testing.
