...
## Purpose
CLI command `lutris eval <file>` for executing JavaScript against .fig files with a Figma-compatible `figma` global object. Enables headless scripting, batch operations, AI tool execution, and testing without GUI. Supports `--code`, `--stdin`, `--write`, `-o`, and `--json` flags.

## Requirements

### Requirement: Execute JavaScript against .fig files

The system SHALL provide `bun lutris eval <file> --code '<js>'` command for headless JavaScript execution.

#### Scenario: Running inline code
- **WHEN** user runs `bun lutris eval design.fig --code 'return figma.currentPage.children.length'`
- **THEN** system loads design.fig, executes code with `figma` global, prints result to stdout

#### Scenario: Returning JSON
- **WHEN** user runs `bun lutris eval design.fig --code 'return { id: figma.currentPage.id }'`
...
- **WHEN** user runs `cat script.js | bun lutris eval design.fig --stdin`
...
- **WHEN** user runs `bun lutris eval design.fig --code 'frame.name = "Updated"' --write`
- **THEN** system modifies design.fig in-place after execution

#### Scenario: Writing to output file
- **WHEN** user runs `bun lutris eval design.fig --code '...' -o modified.fig`
...
- **WHEN** user runs `bun lutris eval design.fig --code '...' --json`
...
- **THEN** eval command appears in `bun lutris --help`

#### Scenario: Command structure
- **WHEN** user runs `bun lutris eval --help`
