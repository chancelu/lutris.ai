...
The app SHALL include a `manifest.webmanifest` file linked from `index.html` with `name` "Lutris.ai", `short_name` "Lutris.ai", `display` "standalone", `start_url` "/", `theme_color` "#1e1e1e", `background_color` "#1e1e1e", and icon entries for 192×192, 512×512, and maskable variants.
