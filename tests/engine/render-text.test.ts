...
import { SceneGraph, SkiaRenderer as SkiaRendererClass } from '@llc3233149/core'
...
describe('renderText headless visual', () => {
  test('renders CJK text via fallback font when node font is unavailable', async () => {
    ...
    let notoPath = new URL('../../tests/fixtures/fonts/NotoSansSC-Regular.ttf', import.meta.url).pathname
    // Windows: URL.pathname returns /C:/... which breaks file access
    if (process.platform === 'win32' && notoPath.startsWith('/')) {
      notoPath = notoPath.slice(1)
    }
