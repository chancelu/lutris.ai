...
- `bun lutris info <file>` — document stats
- `bun lutris tree <file>` — node tree
- `bun lutris find <file>` — search nodes
- `bun lutris node <file> --id <id>` — detailed node properties
- `bun lutris pages <file>` — list pages
- `bun lutris variables <file>` — list design variables
- `bun lutris export <file>` — headless render to PNG/JPG/WEBP
- `bun lutris analyze colors <file>` — color palette usage
- `bun lutris analyze typography <file>` — font/size/weight stats
- `bun lutris analyze spacing <file>` — gap/padding values
- `bun lutris analyze clusters <file>` — repeated patterns
- `bun lutris eval <file> --code '<js>'` — execute JS with Figma Plugin API
...
| `app.yml` | Push to `master` (non-docs) | Build web app, deploy to Cloudflare Pages (`app.lutris.ai`) |
| `docs.yml` | Push to `master` (`packages/docs/**`) | Build VitePress docs, deploy to Cloudflare Pages (`lutris.ai`) |
...
- `packages/docs/` — VitePress site deployed at `lutris.ai`. User guide, reference, development docs.
